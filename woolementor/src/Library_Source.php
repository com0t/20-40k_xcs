...
class Library_Source extends Source_Base {
	 ...
	 */
	const API_CATEGORIES_INFO_URL = WOOLEMENTOR_LIB_URL . '/categories/';
	 ...
	 * Updated api to v2 in version 2.15.0
	 */
	const API_TEMPLATES_INFO_URL = WOOLEMENTOR_LIB_URL . '/list/';

	/**
	 * Template data api url
	 */
	const API_TEMPLATE_DATA_URL = WOOLEMENTOR_LIB_URL . '/get/';

	public function get_id() {
		return 'woolementor-library';
	}

	public function get_title() {
		return __( 'Woolementor Library', 'woolementor' );
	}

	public function register_data() {}

	public function save_item( $template_data ) {
		return new \WP_Error( 'invalid_request', 'Cannot save template to a Woolementor library' );
	}

	public function update_item( $new_data ) {
		return new \WP_Error( 'invalid_request', 'Cannot update template to a Woolementor library' );
	}

	public function delete_template( $template_id ) {
		return new \WP_Error( 'invalid_request', 'Cannot delete template from a Woolementor library' );
	}

	public function export_template( $template_id ) {
		return new \WP_Error( 'invalid_request', 'Cannot export template from a Woolementor library' );
	}
	...
	public function get_data( array $args, $context = 'display' ) {
		...
		if ( empty( $data ) || empty( $data['content'] ) ) {
			throw new \Exception( __( 'Template does not have any content', 'woolementor' ) );
		}
