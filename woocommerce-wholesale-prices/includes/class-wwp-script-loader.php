...
if (!class_exists('WWP_Script_Loader')) {
    ...
    class WWP_Script_Loader
    {
        ...
        public function load_back_end_styles_and_scripts($handle)
        {
            ...
            if (
                $screen->id === 'woocommerce_page_wwp-order-form-page' &&
        ...
        {
            // Check if WWLC is installed / activated
            $plugin                = array();
            $adminNoticeMsg        = "";
            $plugin_wwlc           = 'woocommerce-wholesale-lead-capture/woocommerce-wholesale-lead-capture.bootstrap.php';
            $plugin_name           = explode('/', $plugin_wwlc);
            $plugin['plugin-key']  = $plugin_name[0];
            $plugin['plugin-base'] = $plugin_wwlc;
            $plugin['plugin-name'] = str_replace('Woocommerce', 'WooCommerce', ucwords(str_replace('-', ' ', $plugin_name[0])));

            if (count($plugin) > 0) {

                $adminNoticeMsg = '<a href="https://wholesalesuiteplugin.com/woocommerce-wholesale-lead-capture/?utm_source=freeplugin&utm_medium=upsell&utm_campaign=wwlcshowtononwholesale">Get WooCommerce Wholesale Lead Capture to unlock this powerful feature + more &rarr;</a>';

            }

            // Load CSS for chosen.jquery.min
            wp_enqueue_style('wwp-chosen-jquery-css', 'https://cdn.jsdelivr.net/npm/chosen-js@1/chosen.min.css', array(), $this->_wwp_current_version, 'all');

            wp_enqueue_style('wwp-awesome-font-library-css', 'https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css', array(), $this->_wwp_current_version, 'all');

            wp_enqueue_style('wwp-price-non-wholesale-css', WWP_CSS_URL . 'wwp-price-settings.css', array(), $this->_wwp_current_version, 'all');

            // Load Chosen Jquery for multiselect and search
            wp_enqueue_script('wwp-chosen-jquery-min', 'https://cdn.jsdelivr.net/npm/chosen-js@1/chosen.jquery.min.js', array(), $this->_wwp_current_version, true);

            // Load Custom JS script
            wp_enqueue_script('wwp-price-settings-non-wholesale_js', WWP_JS_URL . 'backend/wwp-price-settings-non-wholesale.js', array('jquery', 'jquery-tiptip'), $this->_wwp_current_version, true);

            wp_localize_script('wwp-price-settings-non-wholesale_js', 'wwp_non_wholesale_settings_js_var', array(
                'wwp_price_settings_register_text'                  => __(get_option('wwp_price_settings_register_text'), 'woocommerce-wholesale-prices'),
                'wwp_price_settings_register_text_tooltip'          => __(wc_help_tip('This text is linked to the defined registration page in WooCommerce Wholesale Lead Capture Settings.'), 'woocommerce-wholesale-prices'),
                'wwp_see_wholesale_prices_replacement_text'         => __(get_option('wwp_see_wholesale_prices_replacement_text'), 'woocommerce-wholesale-prices'),
                'wwp_see_wholesale_prices_replacement_text_tooltip' => __(wc_help_tip('The \'Click to See Wholesale Prices\' text seen in frontend.'), 'woocommerce-wholesale-prices'),
                'wwp_wholesale_role_select_chosen_tooltip'          => __(wc_help_tip('The selected wholesale roles and pricing that should show to non-wholesale customers on the front end.'), 'woocommerce-wholesale-prices'),
                ...
                'wholesale_roles'                                   => $this->_wwp_wholesale_roles->getAllRegisteredWholesaleRoles(),
                'wwp_option_show_wholesale_price_to_non_wholesale'  => get_option('wwp_prices_settings_show_wholesale_prices_to_non_wholesale'),
                'is_wwpp_active'                                    => WWP_Helper_Functions::is_wwpp_active(),
                'wwlc_admin_notice'                                 => $adminNoticeMsg,
                'is_wwlc_active'                                    => is_plugin_active('woocommerce-wholesale-lead-capture/woocommerce-wholesale-lead-capture.bootstrap.php'),
        ...
        public function load_front_end_styles_and_scripts()
        {
            ...
            if (get_option('wwp_prices_settings_show_wholesale_prices_to_non_wholesale') === 'yes' && (current_user_can('manage_woocommerce') || empty($user_wholesale_role))) {
...
<div class="upsell-area">
    ...
    <p>
        ...
        public function load_non_wholesale_users_frontend_styles_and_scripts()
        {
            ...
            $user_wholesale_role                    = $wc_wholesale_prices->wwp_wholesale_roles->getUserWholesaleRole();
            $show_wholesale_prices_to_non_wholesale = get_option('wwp_prices_settings_show_wholesale_prices_to_non_wholesale') ? true : false;

            if ($show_wholesale_prices_to_non_wholesale && (empty($user_wholesale_role) || current_user_can('manage_woocommerce')) && !is_checkout()) {
        ...
        public function run()
        {
            ...
            add_action("wp_enqueue_scripts", array($this, 'load_non_wholesale_users_frontend_styles_and_scripts'), 10);
        }
    }
}