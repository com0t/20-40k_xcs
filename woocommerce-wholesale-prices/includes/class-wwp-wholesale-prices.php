...
class WWP_Wholesale_Prices
{
    ...
    public static function get_product_wholesale_price_on_shop_v3($product_id, $user_wholesale_role)
    {
        ...
        if (
            ...
            if (!empty($price_arr['wholesale_price_raw']) && $price_arr['source'] == 'per_product_level') {
                $price_arr['wholesale_price_raw'] = apply_filters('woocommerce_product_get_price', $price_arr['wholesale_price_raw'], wc_get_product($product_id));
            }
        }
        ...
        if (wc_prices_include_tax() && $price_arr['wholesale_price_raw']) {
            $price_arr['wholesale_price_with_no_tax'] = WWP_Helper_Functions::wwp_get_price_excluding_tax(wc_get_product($product_id), array('qty' => 1, 'price' => $price_arr['wholesale_price_raw']));
        } else {
    ...
    public static function get_wholesale_price_suffix($product, $user_wholesale_role, $wholesale_price, $return_wholesale_price_only = false)
    {
        ...
        if (!empty($user_wholesale_role)) {
            ...
            $price_arr = WWP_Wholesale_Prices::get_product_wholesale_price_on_shop_v3(WWP_Helper_Functions::wwp_get_product_id($product), array('wholesale_customer'));
        ...
        return apply_filters('wwp_filter_wholesale_price_display_suffix', $wc_price_suffix, $product, $user_wholesale_role, $wholesale_price, $return_wholesale_price_only);
    ...
    public function wholesale_price_html_filter($price, $product, $user_wholesale_role = null, $return_wholesale_price_only = false)
    {
        ...
        if ($show_wholesale_prices === 'yes' && (current_user_can('manage_woocommerce') || empty($user_wholesale_role))) {
            if (in_array(WWP_Helper_Functions::wwp_get_product_type($product), array('simple', 'variation', 'variable'))) {
                $price .= WWP_Wholesale_Prices_Non_Wholesales::display_replacement_message_to_non_wholesale();
            }
        }
    ...
    public function display_replacement_message()
    {
        ...
        if ($hide_price_and_add_to_cart_button) {
            ...
            echo apply_filters('wwp_display_replacement_message', $message);
        }
