        ...
        if (!get_option('wwp_tracking_notice') || defined('WWS_TESTING_SITE')) {
          ...
          if (!get_option('wwp_anonymous_data', false)) {
            ...
            if (!WWP_Helper_Functions::is_dev_url(network_site_url('/')) || defined('WWS_TESTING_SITE')) {
              ...
              echo '<p style="font-weight:700;">' . __('Wholesale Suite Usage Tracking Permission') . '</p>';
              echo '<p>';
              echo esc_html__('Allow Wholesale Suite to track plugin usage? Opt-in let us track usage data so we know with which WordPress configurations, themes and plugins we should test with.', 'woocommerce-wholesale-prices');
              echo '</p>';
